...
function ConnectVisiteur($data){
    ...
    $email = $data['email'];
    $mp =md5($data['mp']);
         // Utilisation d'une requête préparée avec des paramètres liés pour éviter les injections SQL
         $requet = "SELECT * FROM visiteurs WHERE email ='$email' AND mp = $mp";
    ...
    function connectAdmin($data){
        $conn = connect();
        $email = $data['email'];
        $mp =md5($data['mp']); // Utilisation de password_hash() pour sécuriser le mot de passe
